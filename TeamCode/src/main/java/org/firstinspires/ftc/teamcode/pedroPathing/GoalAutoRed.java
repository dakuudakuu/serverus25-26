package org.firstinspires.ftc.teamcode.pedroPathing;

import com.pedropathing.follower.Follower;
import com.pedropathing.geometry.BezierCurve;
import com.pedropathing.geometry.BezierLine;
import com.pedropathing.geometry.Pose;
import com.pedropathing.paths.PathChain;
import com.pedropathing.util.Timer;
import com.qualcomm.robotcore.eventloop.opmode.Autonomous;
import com.qualcomm.robotcore.eventloop.opmode.OpMode;
import com.qualcomm.robotcore.hardware.DcMotor;

@Autonomous(name = "GoalAutoRed")
public class GoalAutoRed extends OpMode {

    private Follower follower;
    private Timer pathTimer, actionTimer, opmodeTimer;
    private int pathState;

    private DcMotor rollers;
    private DcMotor wheel1;
    private DcMotor wheel2;

    private double speed = 0.6;

    private final Pose startPose = new Pose(120, 127, Math.toRadians(215.906));
    private final Pose launchPose = new Pose(90, 107, Math.toRadians(215.906));
    private final Pose startGrab1 = new Pose(99, 82, Math.toRadians(0));
    private final Pose finishGrab1 = new Pose(125, 82, Math.toRadians(0));
    private final Pose startGrab2 = new Pose(99, 58, Math.toRadians(0));
    private final Pose finishGrab2 = new Pose(125, 58, Math.toRadians(0));
    private final Pose controlPoint = new Pose (90, 75, Math.toRadians(105));
    private final Pose finishPose = new Pose(110, 82, Math.toRadians(90));

    private PathChain launch1, toPickUp1, pickUp1, launch2, toPickUp2, pickUp2, launch3, leave;

    public void buildPaths() {
        launch1 = follower.pathBuilder()
                .addPath(new BezierLine(startPose, launchPose))
                .setConstantHeadingInterpolation(startPose.getHeading())
                .build();
        toPickUp1 = follower.pathBuilder()
                .addPath(new BezierLine(launchPose, startGrab1))
                .setLinearHeadingInterpolation(launchPose.getHeading(), startGrab1.getHeading())
                .build();
        pickUp1 = follower.pathBuilder()
                .addPath(new BezierLine(startGrab1, finishGrab1))
                .setConstantHeadingInterpolation(startGrab1.getHeading())
                .setGlobalDeceleration(0.3)
                .build();
        launch2 = follower.pathBuilder()
                .addPath(new BezierLine(finishGrab1, launchPose))
                .setLinearHeadingInterpolation(finishGrab1.getHeading(), launchPose.getHeading())
                .build();
        toPickUp2 = follower.pathBuilder()
                .addPath(new BezierLine(launchPose, startGrab2))
                .setLinearHeadingInterpolation(launchPose.getHeading(), startGrab2.getHeading())
                .build();
        pickUp2 = follower.pathBuilder()
                .addPath(new BezierLine(startGrab2, finishGrab2))
                .setConstantHeadingInterpolation(startGrab2.getHeading())
                .setGlobalDeceleration(0.3)
                .build();
        launch3 = follower.pathBuilder()
                .addPath(new BezierCurve(finishGrab2, controlPoint, launchPose))
                .setLinearHeadingInterpolation(finishGrab2.getHeading(), launchPose.getHeading())
                .build();
        leave = follower.pathBuilder()
                .addPath(new BezierLine(launchPose, finishPose))
                .setLinearHeadingInterpolation(launchPose.getHeading(), finishPose.getHeading())
                .setGlobalDeceleration(0.2)
                .build();
    }

    @Override
    public void init() {
        pathTimer = new Timer();
        opmodeTimer = new Timer();
        opmodeTimer.resetTimer();
        follower = Constants.createFollower(hardwareMap);
        follower.setStartingPose(startPose);
        follower.update();
        buildPaths();
        rollers = hardwareMap.get(DcMotor.class, "rollers");
        wheel1 = hardwareMap.get(DcMotor.class, "wheel1");
        wheel2 = hardwareMap.get(DcMotor.class, "wheel2");
    }

    public void pathUpdate() {
        switch(pathState) {
            case 0:
                startLaunch();
                follower.followPath(launch1, speed, true);
                setPathState(1);
                break;
            case 1:
                if(!follower.isBusy()) {
                    setPathState(2);
                }
                break;
            case 2:
                if(pathTimer.getElapsedTimeSeconds() > 1.5) {
                    stopLaunch();
                    follower.followPath(toPickUp1, speed, false);
                    setPathState(3);
                }
                break;
            case 3:
                if(!follower.isBusy()) {
                    startPickUp();
                    follower.followPath(pickUp1, speed, false);
                    setPathState(4);
                }
                break;
            case 4:
                if(!follower.isBusy()) {
                    stopPickUp();
                    startLaunch();
                    follower.followPath(launch2, speed, true);
                    setPathState(5);
                }
                break;
            case 5:
                if(!follower.isBusy()) {
                    setPathState(6);
                }
                break;
            case 6:
                if(pathTimer.getElapsedTimeSeconds() > 1.5) {
                    stopLaunch();
                    follower.followPath(toPickUp2, speed, false);
                    setPathState(7);
                }
                break;
            case 7:
                if(!follower.isBusy()) {
                    startPickUp();
                    follower.followPath(pickUp2, speed, false);
                    setPathState(8);
                }
                break;
            case 8:
                if(!follower.isBusy()) {
                    stopPickUp();
                    startLaunch();
                    follower.followPath(launch3, speed, true);
                    setPathState(9);
                }
                break;
            case 9:
                if(!follower.isBusy()) {
                    setPathState(10);
                }
                break;
            case 10:
                if(pathTimer.getElapsedTimeSeconds() > 1.5) {
                    stopLaunch();
                    follower.followPath(leave, speed, true);
                    setPathState(11);
                }
                break;
            case 11:
                if(!follower.isBusy()) {
                    setPathState(12);
                }
            case 12:
                setPathState(-1);
                stop();
        }
    }

    @Override
    public void loop() {
        follower.update();
        pathUpdate();
    }

    @Override
    public void start() {
        opmodeTimer.resetTimer();
        setPathState(0);
    }

    public void setPathState(int pState) {
        pathState = pState;
        pathTimer.resetTimer();
    }

    public void startLaunch() {
        rollers.setPower(0.9);
        wheel1.setPower(0.65);
        wheel2.setPower(-0.65);
    }

    public void stopLaunch() {
        rollers.setPower(0);
        wheel1.setPower(0);
        wheel2.setPower(0);
    }

    public void startPickUp() {
        rollers.setPower(0.9);
    }

    public void stopPickUp() {
        rollers.setPower(0);
    }

    @Override
    public void stop() {
        PoseStorage.endingPose = follower.getPose();
    }
}